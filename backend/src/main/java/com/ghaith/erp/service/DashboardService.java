...
public class DashboardService {
    ...
    public DashboardSummaryDto getSummary(Long branchId) {
        ...
        DashboardSummaryDto.StatsDto stats = DashboardSummaryDto.StatsDto.builder()
                .hr(hrStats)
                .finance(DashboardSummaryDto.StatsDto.FinanceStats.builder().overdue(0).totalInvoices(0).build())
                .fleet(DashboardSummaryDto.StatsDto.FleetStats.builder().available(0).inMaintenance(0).build())
                .support(DashboardSummaryDto.StatsDto.SupportStats.builder().open(0).critical(0).build())
                .legal(DashboardSummaryDto.StatsDto.LegalStats.builder().openCases(0).expiringContracts(0).build())
    ...
    public List<QuickSearchResultDto> search(String query) {
        ...
        employeeRepository.findAll().stream()
                .filter(e -> (e.getFirstName() + " " + e.getLastName()).toLowerCase().contains(query.toLowerCase()))
                .limit(5)
