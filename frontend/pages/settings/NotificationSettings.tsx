...
export default function NotificationSettings() {
  ...
  const save = async () => { setSaving(true); try { for(const [k,v] of Object.entries(s)) if(k.startsWith('notifications.')) await fetch('/api/trpc/settings.settings.set',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({json:{key:k,value:v}})}); setOk(true);setTimeout(()=>setOk(false),2000); } catch {} setSaving(false); };
