...
export default function PermissionMatrix() {
  ...
  const { data, isLoading, isError, refetch } = trpc.rolePacks.list.useQuery(undefined, {
    retry: 1,
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {!isLoading && !isError && (
        <Card>
          <CardHeader><CardTitle className="flex gap-2">مصفوفة الصلاحيات <Badge variant="secondary">{filtered.length}</Badge></CardTitle></CardHeader>
          <CardContent>
