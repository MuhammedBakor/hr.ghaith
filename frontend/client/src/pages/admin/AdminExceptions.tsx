...
export default function AdminExceptions() {
  const { data, isLoading, isError, refetch } = trpc.governance.getExceptions.useQuery(undefined, {
    retry: 1,
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">الاستثناءات</h1>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              الاستثناءات
              <Badge variant="secondary">{items.length} استثناء</Badge>
            </CardTitle>
          </CardHeader>
          <CardContent>
            {items.length === 0 ? (
              ...
              <div className="divide-y">
                {items.slice(0, 50).map((item: any, i: number) => (
                  <div key={item.id || i} className="py-3 flex items-center justify-between">
                    <div>
                      <p className="font-medium">{item.name || item.title || item.action || item.description || `استثناء #${item.id || i+1}`}</p>
